...
import org.tmt.apsproceduredataservice.TestHelper
import org.tmt.apsproceduredataservice.impl.ApsproceduredataserviceImpl
import org.tmt.apsproceduredataservice.core.models.{AdminGreetResponse, GreetResponse, UserInfo}

import scala.concurrent.Future
import scala.util.Random

class ApsproceduredataserviceRouteTest extends AnyWordSpec with ScalatestRouteTest with PekkoHttpCompat with BeforeAndAfterEach with HttpCodecs {

  private val service1: ApsproceduredataserviceImpl                   = mock[ApsproceduredataserviceImpl]
  private val service2                               = mock[JApsproceduredataserviceImplWrapper]
  ...
  private val route: Route = new ApsproceduredataserviceRoute(service1, service2, securityDirectives).route

  override protected def beforeEach(): Unit = reset(service1, securityDirectives)

  "ApsproceduredataserviceRoute" must {
  ...
  val connection: Connection.HttpConnection = HttpConnection(ComponentId(Prefix(TestHelper.randomSubsystem, "apsproceduredataservice"), Service))
}
