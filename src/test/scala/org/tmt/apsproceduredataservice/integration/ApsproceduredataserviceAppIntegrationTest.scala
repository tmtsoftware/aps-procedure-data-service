package org.tmt.apsproceduredataservice.integration
...
import org.tmt.apsproceduredataservice.impl.ApsproceduredataserviceWiring
import org.tmt.apsproceduredataservice.core.models.{AdminGreetResponse, GreetResponse, UserInfo}
import org.tmt.apsproceduredataservice.http.HttpCodecs

import scala.compiletime.uninitialized
import scala.concurrent.duration.DurationInt
import scala.concurrent.Await

class ApsproceduredataserviceAppIntegrationTest extends ScalaTestFrameworkTestKit with AnyWordSpecLike with Matchers with HttpCodecs {
  ...
  val apsproceduredataserviceAppPort                    = 8085
  val apsproceduredataserviceWiring                     = new ApsproceduredataserviceWiring(Some(apsproceduredataserviceAppPort))
  val appConnection: HttpConnection    = apsproceduredataserviceWiring.settings.httpConnection
  ...
  protected override def beforeAll(): Unit = {
    ...
    apsproceduredataserviceWiring.start(Metadata.empty).futureValue
  ...
  protected override def afterAll(): Unit = {
    ...
    apsproceduredataserviceWiring.stop().futureValue
    super.afterAll()
  }

  "ApsproceduredataserviceWiring" must {

    "start the apsproceduredataservice app and register with location service" in {
