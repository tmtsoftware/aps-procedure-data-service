...
import ApsproceduredataserviceAppCommand.StartOptions
...
object ApsproceduredataserviceApp extends CommandsEntryPoint {
  private val appName: String = getClass.getSimpleName.dropRight(1)
  private val appVersion: String = "0.1.0"
  override def progName: String = "apsproceduredataservice"
    ...
    override def run(command: T, args: RemainingArgs): Unit = {
      command match {
        case StartOptions(port) =>
          val wiring = new ApsproceduredataserviceWiring(Some(port.getOrElse(SocketUtils.getFreePort)))
